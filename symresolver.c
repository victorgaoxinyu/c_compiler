...
struct symbol *symresolver_get_symbol(struct compile_process *process, const char *name)
{
    ...
    while (symbol)
    {
        ...
        symbol = vector_peek_ptr(process->symbols.table); // vector_at?
    }
...
{
    // symbol cannot share the same name
...
struct node *symresolver_node(struct symbol *sym)
{
    ...
    return sym->data; // the data should contains a struct node
}
...
{
    compiler_error(process, "Structures are not yet supporeted\n");
}
