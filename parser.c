...
void parse_datatype_modifiers(struct datatype *dtype)
{
    ...
    while (token && token->type == TOKEN_TYPE_KEYWORD)
    {
        ...
        {
            dtype->flags &= ~DATATYPE_FLAG_IS_SIGNED; // reset the flag
        }
...
int parser_get_random_type_index() // not actually random
{
...
void parser_datatype_init_type_and_size_for_primitive(struct token *datatype_token, struct token *datatype_secondary_token, struct datatype *datatype_out)
{
    ...
    else if (S_EQ(datatype_token->sval, "long"))
    {
        ...
        datatype_out->size = DATA_SIZE_DWORD; // 32-bit compiler, need to split for double
    }
...
{
    // sanity check, as we cannot do `struct int`
    ...
    case DATA_TYPE_EXPECT_UNION:
        compiler_error(current_process, "Struct and Union types are not supported yet\n");
...
{
    struct token *datatype_token = NULL;           // long int
    struct token *datatype_secondary_token = NULL; // datatype secondary datatype

    parser_get_datatype_tokens(&datatype_token, &datatype_secondary_token);             /* struct abc */
    int expected_type = parser_datatype_expected_for_type_string(datatype_token->sval); /* datatype_token will be set to struct */
                                                                                        /* datatype_secondary_token still NULL  */
    if (datatype_is_struct_or_union_for_name(datatype_token->sval))                     /* expected type is STRUCT              */
    {
        // need to parse name of struct or union
        // struct abc
        // {
        //    int x;
        // } abc;
        ...
        {
            datatype_token = token_next(); // datatype_token is no longer `struct` but `abc`
        }
        else
        // struct
        // {
        ...
        // } abc;
        {
            // this struct has no name, we need to handle it
...
void parse_datatype(struct datatype *dtype)
{
    ...
    // in C, all vars are signed by default
    dtype->flags |= DATATYPE_FLAG_IS_SIGNED;
...
/**
 * @brief ignore int in things like long int abc
 *
 * @param dtype
 */
...
void parser_ignore_int(struct datatype *dtype)
{
    ...
    {
        compiler_error(current_process, "You provided a secondary \"int\" type, however it's not supported with the current abbrevation\n");
    }
...
// by the time we know we are parsing a function
// we should already know its datatype and function name
// imaging parsing `int a` and `int a()`, the second one is a function
void parse_function(struct datatype* ret_type, struct token* name_token, struct history* history)
{
    ...
    if (datatype_is_struct_or_union(ret_type))
    {
        ...
        #warning "Need to recheck this part"
    }
    ...
    // this is also responsible for parsing prototypes
...
void parse_body_single_statement(size_t* variable_size, struct vector* body_vec, struct history* history)
{
    ...
    struct node* stmt_node = NULL; // statement node
...
void parse_variable_function_or_struct_union(struct history *history)
{
    ...
    // Ignore integer abbrv if neccessary i.e "long int" becomes just "long"
    parser_ignore_int(&dtype);

    // int abc <- parse abc now
    struct token *name_token = token_next();
    if (name_token->type != TOKEN_TYPE_IDENTIFIER) // int 49 or other stupid stuff
    {
        compiler_error(current_process, "Expecting a valid name for the given variable declaration\b");
    }

    // Check if this is a function declaration like int abc()
